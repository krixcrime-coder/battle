...
private const val PREFS_NAME = "rush47_prefs"
        ...
        ApiClient.instance.login(body).enqueue(object : Callback<ApiResponse> {
            override fun onResponse(call: Call<ApiResponse>, response: Response<ApiResponse>) {
                ...
                if (result?.success == true) {
                    ...
                    // Backend ke data object me token + user info aata hai (login.php dekho)
                    val data = result.data?.asJsonObject
                    val prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
                    prefs.edit().apply {
                        putString("auth_token", data?.get("token")?.asString)
                        putString("first_name", data?.get("first_name")?.asString)
                        putString("last_name", data?.get("last_name")?.asString)
                        putString("username", data?.get("username")?.asString)
                        putString("email", data?.get("email")?.asString)
                        apply()
                    }
